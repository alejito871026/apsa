  ...
  <div class="p-4 text-center">
    <div class="card">
        <div class="card-header bg-white">
          <h4>Egresos pagos a terceros</h4>
        </div>
                ...
                <thead class="sticky-top text-white bg-dark">
                    ...
                    <tbody>
                        <tr v-for="(egreso, index) in egresos" :key="index">
                            ...
                            <td>{{egreso.entidad}}</td>
                            <td>{{verFecha(egreso.fecha)}}</td>
                        </tr>
                    </tbody>
            </table>
        </div>
...
export default {
    ...
    methods : {
        ...
        async paginacion(posicion){
            ...
            let paginas = await this.$axios.$post("/contabilidad/totalEgresosTerceros")
        ...
        async cambiarPagina (pagina) {
            ...
            let egresos = await this.$axios.$post("/contabilidad/egresosTerceros", pag)
        ...
        async cargarIngresos(){
            let pagina = 1
            let egresos = await this.$axios.$post("/contabilidad/egresosTerceros",pagina)
