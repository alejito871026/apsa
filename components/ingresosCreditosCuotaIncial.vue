  ...
  <div class="p-4 text-center">
    <div class="card">
        <div class="card-header bg-white">
          <h4>Ingresos creditos cuota inicial</h4>
        </div>
                ...
                <thead class="sticky-top text-white bg-dark">
                    ...
                    <tbody>
                        <tr v-for="(ingreso, index) in ingresos" :key="index">
                            ...
                            <td>{{ingreso.descripcion +' '+ ingreso.idCredito.servicio+' '+ingreso.idCredito.nombreCliente+' '+ingreso.idCredito.numeroCredito}}</td>
...
export default {
    ...
    methods : {
        ...
        async paginacion(posicion){
            ...
            let paginas = await this.$axios.$post("/contabilidad/totalIngresosCreditosInicial")
        ...
        async cambiarPagina (pagina) {
            ...
            let ingresos = await this.$axios.$post("/contabilidad/ingresosCreditosInicial", pag)
        ...
        async cargarIngresos(){
            let pagina = 1
            let ingresos = await this.$axios.$post("/contabilidad/ingresosCreditosInicial",pagina)
